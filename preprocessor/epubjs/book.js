...
const {extend, defer} = require("./utils/core");
...
const { EPUBJS_VERSION, EVENTS } = require("./utils/constants");
...
class Book {
	...
	load(path) {
		...
		if(this.archived) {
            ...
            if (!path.includes(this.url.directory)) {
                path = jspath.join(this.url.directory, path);
            }
            
            return new Promise(function(resolve, reject) {

                require("fs").promises.readFile(path, 'utf8')
                    .then(function(text) {
                    ...
                    .catch(error => {
                        if (error.toString().indexOf('com.apple.ibooks.display-options') === -1) {
                            // console.log(error);
